...
// Handhabung des Handout-Downloads (füge das am Ende deines JS-Codes hinzu)
const downloadLink = document.getElementById('menuDownload');
downloadLink.addEventListener('click', function(event) {
    event.preventDefault();
    const fileUrl = 'Handout-IT-Tapio.png';
    const filename = 'Handout-IT-Tapio.png';
    fetch(fileUrl)
        .then(response => {
            if (!response.ok) {
                throw new Error('Netzwerkantwort war nicht ok');
        ...
        .catch(error => {
            console.error('Fehler beim Herunterladen:', error);
            alert('Fehler beim Herunterladen. Starte einen lokalen Server!');
        });
})
